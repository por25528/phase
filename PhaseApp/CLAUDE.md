...
- `src/db/calendarCache.ts` — the only module that touches the `calendarCache` table. Same rule as `assets.ts` and for the same reason: these writes are surgical and must never be folded into `persist()`, which is a full clear + bulkPut of the four app-data tables.
...
- `src/lib/sync/*` + `src/state/syncIngest.ts`, `src/state/syncExport.ts` — the file contract with the PhasePhone companion. The Mac is the ONLY writer of `state.json` and the phone the only writer of `ops-phone.jsonl`; ingest maps each op onto `handleAgentWrite`, so the companion opens no new door into the store. `src/lib/sync/replay.ts` is the phone's projection and every branch of it MIRRORS the matching branch of `agentWrites` — including the day it stamps. That day is `opDay(op)` and NOT either side's clock: the phone records its own local day on the op (`CompanionOp.day`), and the ingest passes it to `handleAgentWrite` as `deps.today`, which the three day-stamping actions (`toggleLeaf`, `toggleTask`, `setNodeStatus`) take as an optional trailing argument defaulting to `todayStr()`. Without that, an op made at 23:50 and ingested at 00:10 lands on a different day than the phone already drew it under.
- **Google Calendar is a producer in `electron/` and read-only derived state in `src/`.** The main process owns OAuth (PKCE, a one-shot loopback listener, `safeStorage`), the Google client, and event normalisation; the renderer sees `BusyBlock[]` and nothing else — no token, no client secret, ever, in either direction. `src/lib/calendarBridge.ts` is the typed, TOTAL accessor for `window.phaseCalendar` and returns `null` in a browser, so absence is a type the caller must handle rather than a throw at the first `await`. `src/lib/calendarCacheState.ts` decides when a cached range may still be believed (account, calendar selection and machine timezone must all still match); a mismatch discards the blocks from memory and LEAVES the row on disk, because `status()` can fail transiently and deleting on that would silently cost a fetched fortnight. **The one exception is `reauth-required`**, which deletes the row: its provenance all still matches, so "re-evaluate next boot" would have the next launch believe a fortnight of blocks no token can refresh. `src/lib/calendarHealth.ts` turns the whole state into one caveat that names the FIX, and owns the two staleness thresholds — fifteen minutes earns a refetch on focus and no words, two hours earns `calendar may be out of date`, and a transient fetch failure over a stale cache earns `calendar didn't refresh`. The thresholds live in `lib` because **`src/lib` never imports `src/views`**; the hook imports them, not the other way round. `WeekHeader` renders the finished string and does not derive it.
- **Three things about fetching that are not obvious.** `refreshCalendar` is gated on the tab lock BEFORE the fetch, not merely before the write — a second tab must not spend a Google quota. Two refreshes in flight is ordinary (Settings' Refresh and the planner's navigate effect fire independently) and both hazards are guarded: `calendarFetchSeq` drops a superseded result so it cannot land last and undo a newer one, and `calendarPendingEnd` remembers the widest request still in flight so a second call cannot ask for LESS than the first and shrink a covered range. `state.calendarWeek` is the week the planner published; a bare `refreshCalendar()` — Connect, and Settings' Refresh — anchors on it, without which a connect made on week +30 left that week permanently caveated. And `outsideHorizon` in `calendarRange.ts` stops the planner chasing a week outside EITHER edge — `fetchRange` clamps forward at `MAX_FORWARD_DAYS` and never grows backward, so every retry returns the identical range. It returns the DIRECTION rather than a boolean, because the two edges need different sentences: `calendar reaches six months out` one way and `calendar only reaches one week back` the other, and a single flag had the header saying the first about last month.
- **A failed fetch KEEPS the blocks it had.** Presenting a booked week as free because the network hiccuped is worse than showing data minutes old; only `not-connected` and `reauth-required`, which mean the data is genuinely gone, clear them. **The build may ship its own OAuth client** (`electron/calendarCredentials.cjs`, written by `npm run calendar:credentials` into a git-ignored file, or read from `PHASE_GOOGLE_*` in dev); a user-saved client always wins over it, and switching either way revokes the old grant BEFORE forgetting the credentials it is revoked with — the local store holds the only copy of the refresh token. A stored token records the client id that issued it, so a rotated managed client surfaces as `reauth-required` before a request is even spent; and a refused refresh reading `invalid_client`, `unauthorized_client` or a bare 401 maps to the same verdict and drops the token, which is what covers the two rotations the stamp cannot see — a token stored before the field existed, and a custom client whose secret was rotated in the Cloud console. Everything else a refresh can fail with stays transient, because prompting for a reconnect that would not fix it teaches people to ignore the prompt. Nothing in the repository holds a credential.
- `electron/main.cjs` — desktop shell (BrowserWindow, dev-server/dist URL switch).
- **The main frame may become exactly two documents, and the preload is why.** `electron/navigationPolicy.cjs` is the one vocabulary — `navigationDecision` answers `internal` / `external` / `block`, and `applyNavigationPolicy` installs it on `will-navigate`, `will-frame-navigate` and `setWindowOpenHandler` together. A preload SURVIVES a top-level navigation, and this one exposes `phaseAgent`, `phaseBackups`, `phaseCalendar`, `phaseSync` and `phaseShell` — so a renderer talked into setting `location` would hand the store's whole write surface, the backup folder and the calendar's tokens to a remote origin, with `contextIsolation` on and helping not at all. The whitelist is the dev server's ORIGIN (never a prefix: `http://localhost:5173.evil.com` and `http://evil-localhost:5173` both pass a `startsWith`) or the packaged entry's `pathname` (never the whole href, or an ordinary reload carrying a hash would be refused) — and exactly one of the two is live, so a `file://` target is blocked in dev and the dev origin is external in a packaged build. Everything else is `block`, INCLUDING what will not parse: a URL we cannot reason about is not one to allow. `block` reaches `shell.openExternal` never — handing a refused `javascript:` target to the shell is how a blocked navigation becomes a worse one — and the window open handler now denies UNCONDITIONALLY, because Phase has no second in-app window to offer. It replaced a handler whose default was inverted: `startsWith('http://')` then `{ action: 'allow' }`, so `file:`, `data:` and every custom scheme got a real window with the real preload in it. The assistant shelf takes the same policy through an injected `guardNavigation`, so the controller keeps owning the window's lifecycle and nothing else.
...
- **A topic is rated, never ticked.** A leaf under a node carrying `topics: true` is a topic: `src/lib/confidence.ts` is its one vocabulary (`topicIds`, `topicConfidence`, `sortForReview`, `readiness`), and `rateTopic` is the only writer of `confidence`/`confidenceAt`, which are written and cleared together (`migrateNodeStatus` drops half a pair on load and import). A topic never carries `status: 'done'` — `toggleLeaf`, `setNodeStatus('done')`, the bulk `setNodesStatus`/`completeNodes`, `finishWork`, the agent's `complete_task`/`set_status` and the phone's replay all refuse it; `doing`, `blocked` and `parked` still move attention exactly as before. Its fraction in `pct.ts` is `CONFIDENCE_WEIGHT` (a legacy tick on it is NOT read); `leafCount` counts it open forever, so a subject never reads `ready-to-complete` and stays on the rail until archived; `goalEffort` leaves it out of every figure and states `readiness` instead, which `effortCount`, `describeEffort` and `effortPct(e, g)` print and draw as one number. Its `due` in `backlogGroups` is the CONFIRMED exam date, which is how the nearest exam's weakest topic leads every surface through `sortByDue` with no second ranking — `sortForReview` ranks inside one subject only. The rating moment is the focus draft's `'rating'` phase — entered only after a sitting on a topic is LOGGED (`completeFocus`, `confirmFocus(minutes)`; never `confirmFocus(null)`, never `finishWork`), answered by `rateFocus`, treated as not-running by the tray, the pill, the idle watcher and `reconcileFocusDraft` — and its known cost is that the rating's undo sweeps the log's. The tree row draws `ConfidenceBox` (a readout, `role="img"`) where a step has its box, and every rail, Today and shelf row draws `ConfidenceMark` where it would draw a tick; the task page's Confidence line is the one manual rating surface, there to correct a mis-tap. The shelf's `rating` card is the tallest state at every width (see the `HEIGHT` ledger in `electron/assistantWindow.cjs`): its question names the title, so it wraps with it, and `WorkBand`'s `stack` is what keeps that under a third of the window at 520.
...
- **A backup that did not happen has to SAY so, on a surface someone reads.** The scheduler's only report was `deps.logError` — a `console.warn` — so a folder failing every half hour was invisible while Backups Settings went on describing versioned copies being kept, and the one moment that promise mattered was the moment it turned out to be false. `AutoBackupDeps.onOutcome(ok)` is the fix and it is a bare BOOLEAN: the scheduler knows whether the last attempt landed and nothing else, and the words belong to the surfaces. It fires on every attempt, automatic and flushed alike, so a manual snapshot that works clears the warning — a working folder is a working folder. `state.autoBackupFailed` latches it, `set` and never `setAndPersist` (a whole-database write on every backup outcome would be the tail wagging the dog), and it is EPHEMERAL for the same reason `persistFailed` is: it describes this session's disk, not the user's data. Two readers, because one is not enough: the app banner, which is the only thing that reaches someone who never opens Settings, and `BackupsSettings`' own heading, which SWAPS its blurb rather than adding a line — a section promising versioned copies above a list that stopped a month ago is the exact failure the warning exists to prevent. `not-owner` sets NOTHING: another window's turn is not a disk problem, and reporting it as one sends someone hunting free space over a second tab being open.
- **The fatal screen may say a download STARTED and never that a file was saved.** `downloadBackupText` clicks an anchor; nothing observes the destination, a cancelled save prompt, or a full disk, in Electron or in a browser. `ErrorBoundary` answered `Saved to your downloads`, which is a claim it cannot make, on the one surface whose whole job is to not lie about whether data is safe — an over-confident success leaves someone reloading in the belief they have a copy exactly as a silent failure would. The state is `started`, not `saved`, the copy names the file, tells the user to finish any save prompt and says the outcome is unconfirmed. The blob URL is revoked after `BLOB_URL_REVOKE_MS` and never in the click's own task: revoking synchronously cancels the download in WebKit, so the choice was a leak or a delay, and a minute of memory that was already allocated is the cheaper one.
