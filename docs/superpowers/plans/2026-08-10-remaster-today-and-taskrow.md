...
**Risk.** The stretched overlay covers trailing metadata for pointer events. `meta` is deliberately not raised above it, so metadata is not clickable and a click anywhere on it opens the row — that is the intent. Nothing interactive may be placed in `meta`: a control needing its own click target must be passed as `lead`, which is raised with `relative z-10`, or the component needs a new explicitly raised slot. If this is ignored, clicking such a control on the offer list falls through to `onOpen`, which calls `place(...)` and performs a persisted schedule write with no undo.
