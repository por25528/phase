...
- **`@testing-library/jest-dom` is NOT installed.** Matchers like `toHaveValue`, `toBeInTheDocument` and `toHaveTextContent` do not exist. Assert with plain DOM reads (`(el as HTMLInputElement).value`, `expect(el).toBeTruthy()`, `el.textContent`) as every existing test in this repo does.
...
- [ ] **Step 3b: Hoist the grouping loop so there is exactly one copy**

`rowActionGroups` already contains this loop. Two copies would drift the first
time the rule changed, so define the helper once, directly above `rowActionGroups`:
...
and replace `rowActionGroups`' body — keeping its existing docstring — with:
...
The existing `rowActionGroups` tests must stay green with no edit. That is the proof the hoist changed nothing.
...
- Create: `src/components/StatusMark.tsx` (moved out of `StepPanel`, Step 5)
- Modify: `src/views/project/StepPanel.tsx` (import the moved `StatusMark`, Step 5)

**Interfaces:**
- Consumes: `Popover` from `./Popover`; `IconCheck`/`IconCircle`/`IconDiamond` from `./Icons`.
- Produces: `export function StatusMark({ status }: { status: StepStatus }): JSX.Element`
- Produces:
  ```ts
...
- [ ] **Step 5: Lift `StatusMark` out of `StepPanel` so both surfaces share one**

`StepPanel` keeps using it (a container's derived status) and `TaskPage` needs it (the status chip). Copying it would be two drawings of the same four states.

Create `src/components/StatusMark.tsx`, moving the function and its docstring verbatim from `src/views/project/StepPanel.tsx:77–91`:
...
Then in `src/views/project/StepPanel.tsx`: delete the local `StatusMark` definition and add `import { StatusMark } from '../../components/StatusMark';`. Drop any icon import that `StepPanel` no longer uses — `noUnusedLocals` will name them.

> Do **not** touch `src/components/SectionLabel.tsx`. It exists, is imported by nobody, and carries different metrics (`mb-3`, `mt-[34px]`) than the local labels — reconciling it is not this feature's job.

- [ ] **Step 6: Run the tests**

Run: `npx vitest run src/components/PropertyRow.test.tsx src/views/project/StepPanel.test.tsx`
Expected: PASS. `StepPanel.test.tsx` must be green with no edit — that is the proof moving `StatusMark` changed nothing.
...
```bash
git add src/components/PropertyRow.tsx src/components/PropertyRow.test.tsx src/components/StatusMark.tsx src/views/project/StepPanel.tsx
